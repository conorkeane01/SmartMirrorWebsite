...
export function GlobalContextProvider(props) {
    const [globals, setGlobals] = useState({ aString: 'init val', count: 0, hideHamMenu: true, notings: [], dataLoaded: false })

    useEffect(() => {
        getAllNotings()
    }, []);

    async function getAllNotings() {
        const response = await fetch('/api/get-notings', {
            method: 'POST',
            body: JSON.stringify({ notes: 'all' }),
            headers: {
                'Content-Type': 'application/json'
            }
        });
        let data = await response.json();
        setGlobals((previousGlobals) => { const newGlobals = JSON.parse(JSON.stringify(previousGlobals)); newGlobals.notings = data.notings; newGlobals.dataLoaded = true; return newGlobals })
    }
    ...
    async function editGlobalData(command) { // {cmd: someCommand, newVal: 'new text'}
        ...
        if (command.cmd == 'addNote') {
            ...
            setGlobals((previousGlobals) => {
                const newGlobals = JSON.parse(JSON.stringify(previousGlobals))
                newGlobals.notings.push(command.newVal); return newGlobals
            })
        }
    }
